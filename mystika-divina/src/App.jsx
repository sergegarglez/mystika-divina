...
// ── Stripe config ─────────────────────────────────────────────────────────────
const STRIPE_PRODUCT_ID   = "prod_Uk3ZaCMp9C0sxZ";
const STRIPE_PUBLISHABLE  = "pk_test_51TkZLk374aLVRteypGaiWmyk87n96bAytk1glAhOJ3G8P8WhkR4F3e3RrHfbnAs7xQOBCv8NufUtJCK01knjNrGR00RPFKmM1R";
const STRIPE_PRICE_ID     = "price_1TkZRy374aLVRteytf9GFE6S";
// Payment Link: create at dashboard.stripe.com → Payment Links → select product → Copy link
// Format: https://buy.stripe.com/test_XXXXXXXX
const STRIPE_PAYMENT_LINK_URL = "https://buy.stripe.com/test_9B614g6DK6dX0qh6Th8EM00";  // ✅ Stripe Payment Link
...
// Simple hash (demo — not cryptographically secure)
function hashPwd(s) {
  let h = 0x811c9dc5;
...
async function dbGetUser(u) { const r = await sg(`u:${u.toLowerCase()}`); return r ? JSON.parse(r) : null; }
...
function AuthModal({ onLogin, onClose }) {
  ...
  const doLogin = async () => {
    ...
    if (!u || u.passwordHash !== hashPwd(password)) { setErr("Usuario o contraseña incorrectos."); setLoading(false); return; }
  ...
  const doRegister = async () => {
    ...
    if (username.length < 3) { setErr("El usuario debe tener al menos 3 caracteres."); return; }
    ...
    const u = { username, email, passwordHash: hashPwd(password), isPremium: false, createdAt: Date.now() };
    await dbSaveUser(u);
    await dbSetSession(u.username);
...
function PlansModal({ user, onClose, onUpgrade }) {
  ...
  const activatePremium = async () => {
    ...
    try {
      ...
      setErr("Error al activar: " + (e.message || e));
    } finally {
      setLoading(false);
    }
  };
...
export default function CartaNatalApp() {
  ...
  useEffect(() => {
    ...
    dbGetSession().then(async u => {
      if (u) {
        const qc = await dbGetQCount(u.username);
        // Auto-activate premium if returning from successful Stripe checkout
        if (payStatus === "success" && !u.isPremium) {
          const upgraded = { ...u, isPremium: true, premiumSince: Date.now() };
          await dbSaveUser(upgraded);
          setUser({ ...upgraded, questionsUsed: 0 });
          // Clean URL
        ...
        // User returned from Stripe but no session — prompt to log in
        setShowAuth(true);
      }
    ...
    <div style={{ minHeight: "100vh", background: "#141229", color: "#EDE7F6", fontFamily: "'Jost', system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "28px 16px 60px" }}>
        ...
        <header style={{ textAlign: "center", marginBottom: 26 }}>
          ...
          <p style={{ margin: 0, color: "#8B7FB8", fontSize: 14, fontStyle: "italic", fontFamily: "'Cormorant Garamond', serif", fontSize: 17 }}>
